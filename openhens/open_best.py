...
if best.parent.framework == 'ESM': # best soln is ESM
    ...
    TDM = best.parent  
    PDM_grid = Grid_Diagram(network=PDM.case, non_iso=False, draw_stages=False, draw_HP=False, comparison_network=best.case)  # Create grid diagram for PDM case
    TDM_grid = Grid_Diagram(network=TDM.case, non_iso=False, draw_stages=False, draw_HP=False, comparison_network=best.case)
elif best.parent.framework == 'TDM':# best soln is TDM (very unlikely)
    PDM = best.parent
else: # best soln is PDM (shouldnt occur)
    pass
 
best_grid = Grid_Diagram(network=best.case, non_iso=True, draw_stages=False, draw_HP=False, comparison_network=None)  # Create grid diagram for TDM case
